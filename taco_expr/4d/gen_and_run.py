...
results_filename = 'taco_4d_ssss_iojo_k21.csv'
def process_mat(mtx, indir, outdir, start, end, copy_freq):
    ...
    for nb in (2**index for index in range(start, end+1)):
        for mb in (2**index for index in range(start, end+1)):
            ...
            apptainer_cmd = f'apptainer exec --bind {outdir}:/data taco_py_latest.sif /data/mod_run_tool_4d.sh /data/ /data/{results_filename} 10'
