...
std::vector<float> SpMV(const CSRMatrix& tile, const std::vector<float>& denseVec, int colOffset){
...
void SpMVTiled(const std::vector<CSRMatrix>& csrBlocks, const std::vector<float>& denseVec, std::vector<float>&result, int numRows, int numCols, int rowsPerBlock, int colsPerBlock){
    ...
    for (size_t blockIdx = 0 ; blockIdx < csrBlocks.size(); ++blockIdx){
        ...
        std::vector<float> tileResult = SpMV(csrBlocks[blockIdx], denseVec, colOffset);
...
int main() {
    ...
    SpMVTiled(csrBlocks, denseVec, result_ref, numRows, numCols, rowsPerBlock, colsPerBlock);

    rowsPerBlock = 4;
    colsPerBlock = 4;
    csrBlocks = tileAndConvertToCSR(rows, cols, values, numRows, numCols, rowsPerBlock, colsPerBlock);
    SpMVTiled(csrBlocks, denseVec, result, numRows, numCols, rowsPerBlock, colsPerBlock);
