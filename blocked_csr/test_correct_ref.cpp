...
int main() {
  ...
  std::vector<int> rowPtrs(numRows+1, 0);
  std::vector<int> colIndices(numNZ);
  std::vector<float> vals(numNZ);
  coo_to_csr_scipy_v(numRows, numCols, numNZ, rows, cols, values, rowPtrs, colIndices, vals);

  std::vector<float> result_ref(numRows, 0);
  spmv_ref_csr_v(rowPtrs, colIndices, vals, numNZ, numRows, denseVec, 1.0f, 0.0f, result_ref);
